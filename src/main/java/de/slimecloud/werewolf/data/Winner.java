...
public enum Winner {
	LOVER(Player::isLover,
			game -> game.getPlayers().values().stream().filter(Player::isAlive).filter(Player::isLover).count() >= game.getPlayers().values().stream().filter(Player::isAlive).filter(p -> !p.isLover()).count() && game.getPlayers().values().stream().filter(Player::isAlive).filter(p -> !p.isLover()).noneMatch(p -> p.getRole().isKilling())),
